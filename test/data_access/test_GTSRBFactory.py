...
@patch(
    "data_access.datasets.GTSRBFactory.ConceptAwareDataset",
    return_value=Mock(spec=ConceptAwareDataset, __len__=Mock(return_value=100)),
)
def test_load_datasets_calls_correct_classes(
    MockConceptAwareDataset, mock_random_split, factory, mock_root_dirs, mock_config
):
...
@patch(
    "data_access.datasets.GTSRBFactory.ConceptAwareDataset",
    return_value=Mock(__len__=Mock(return_value=100)),
)
@patch(
    "data_access.datasets.GTSRBFactory.random_split",
    return_value=[
        Mock(__len__=Mock(return_value=80)),
        Mock(__len__=Mock(return_value=20)),
    ],
)
def test_load_datasets_with_custom_transforms(
    mock_random_split, MockConceptAwareDataset, factory
):
    ...
    factory.load_datasets(
        train_transform=custom_train_transform, test_transform=custom_test_transform
    )
...
def test_load_datasets_sets_datasets_loaded_flag(
    mock_random_split, MockConceptAwareDataset, factory
):
...
def test_load_datasets_handles_no_concepts_file(
    mock_random_split, MockConceptAwareDataset, factory, mock_config
):
