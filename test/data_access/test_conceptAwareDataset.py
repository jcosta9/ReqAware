...
def test_init_baseline_mode_no_concepts_file(mock_image_open, setup_mock_filesystem, mock_transform):
...
@patch('PIL.Image.open', return_value=Image.new('RGB', (10, 10)))
def test_init_concept_aware_mode(mock_image_open, setup_mock_filesystem, mock_transform):
...
@patch('PIL.Image.open', return_value=Image.new('RGB', (10, 10)))
def test_init_with_nonexistent_concepts_file_raises_error(mock_image_open, setup_mock_filesystem, mock_transform):
    ...
    with pytest.raises(FileNotFoundError):
        ConceptAwareDataset(
            root_dir=root_dir, 
            transform=mock_transform, 
            concepts_file=non_existent_file
        )
...
def test_get_images_labels_paths_handles_non_integer_dirs(setup_mock_filesystem, mock_transform, caplog):
