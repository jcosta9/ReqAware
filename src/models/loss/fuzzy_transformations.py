...
class LogProductAAggregation(Aggregation):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if inputs.dim() < 2:
            raise ValueError(
                "LogProductAggregation expects at least a 2D tensor [batch, instances]"
            )
...
class GeneralizedMeanEAggregation(Aggregation):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        if inputs.dim() < 2:
            raise ValueError(
                "GeneralizedMeanAggregation expects at least a 2D tensor [batch, instances]"
            )
...
class ProbSumAggregator(Aggregation):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        if inputs.dim() < 2:
            raise ValueError(
                "ProbSumAggregator expects at least a 2D tensor [batch, instances]"
            )
...
class ProductAAggregation(Aggregation):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        if inputs.dim() < 2:
            raise ValueError(
                "Product aggregation expects at least a 2D tensor [batch, instances]"
            )
