    ...
    def forward(self, y_pred: torch.Tensor) -> torch.Tensor:
        ...
        if num_concepts < 4:
            ...
            indices = torch.combinations(
                torch.arange(num_concepts, device=y_pred.device), r=4
            )
...
class NoSymbolsExactlyTwoColours(FuzzyLoss):
    ...
    def forward(self, y_pred: torch.Tensor) -> torch.Tensor:
        ...
        indices = torch.triu_indices(
            num_colours, num_colours, offset=1
        )  # gets all row and col indices from matrix diag that is offset by one
        ...
        if num_colours < 3:
            ...
            indices = torch.combinations(
                torch.arange(num_colours, device=y_pred.device), r=3
            )
        ...
        not_exactly_two_colours = self.t_conorm(
            has_three_or_more, has_zero_or_one_colour
        )
...
class WarningSignExclusivity(FuzzyLoss):
    ...
    def __init__(
        ...
        if (
            (len(self.warning_indices) == 0)
            or (len(self.symbol_indices) == 0)
            or not (set(self.warning_indices) <= set(self.symbol_indices))
        ):
            raise ValueError(
                "WarningSignExclusivity requires a set of warning and symbol indices and warning needs to be a subset of symbols."
            )
